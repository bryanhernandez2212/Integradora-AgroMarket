...
// Función para calcular y mostrar precio con descuento (definida globalmente antes de cualquier uso)
window.actualizarPrecioConDescuento = function() {
...
async function cargarProducto() {
    ...
    const descuentoInput = document.getElementById('descuento');
    if (descuentoInput) {
        descuentoInput.value = productoActual.descuento || 0;
        // Llamar a la función de actualización (ahora está disponible globalmente)
        setTimeout(() => {
            window.actualizarPrecioConDescuento();
        }, 100);
    }
...
async function actualizarProducto(datos) {
    ...
    const precioConDescuento = descuento > 0 ? precioOriginal * (1 - descuento / 100) : null;
...
function setupValidaciones() {
    ...
    if (precioInput && precioError && precioErrorTexto) {
        ...
        precioInput.addEventListener('input', () => {
            ...
            if (typeof window.actualizarPrecioConDescuento === 'function') {
                window.actualizarPrecioConDescuento();
            }
        });
        ...
        const descuentoInput = document.getElementById('descuento');
        if (descuentoInput) {
            descuentoInput.addEventListener('input', window.actualizarPrecioConDescuento);
            descuentoInput.addEventListener('change', window.actualizarPrecioConDescuento);
        }
    }
}
